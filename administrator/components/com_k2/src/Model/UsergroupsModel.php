...
class UsergroupsModel extends ListModel
{
    ...
    protected function getListQuery()
    {
        ...
        $query->select([
            $db->quoteName('a.id'),
            $db->quoteName('a.name'),
            $db->quoteName('a.permissions'),
        ]);

        $query->from($db->quoteName('#__k2_user_groups', 'a'));

        // Subquery to count users in each group
        $subQuery = $db->getQuery(true)
            ->select('COUNT(*)')
            ->from($db->quoteName('#__k2_users', 'u'))
            ->where($db->quoteName('u.group') . ' = ' . $db->quoteName('a.id'));
        $query->select('(' . $subQuery . ') AS ' . $db->quoteName('numOfUsers'));
